...
def create_account() -> None:
    ...
    try:
        ...
        if checker1 is not None:
            raise Error(
                "A user with this login " +
                "is already registered")
    except db.db_Error as e:
        raise Error(str(e))

    password: str = input("Enter your password: ")
    if len(password) < 1 or not isinstance(password, str):
...
def auth() -> Tuple[str, bytes]:
    ...
    if len(error_list) != 0:
        raise Error(
            f"'{''.join(error_list)}' is not allowed")
    ...
    while attempt != 3:
        ...
        try:
            if choice == 'Y' or choice == 'y':
                ...
                attempt += 1
                print(f"Invalid answer! {attempt} / 3")
                continue
        except ValueError:
            attempt += 1
            print(f"Invalid answer! {attempt} / 3")
...
def _note_name() -> Union[str, int]:
    ...
    for elem in note_name:
        if elem not in _ALLOWED_CHARACTERS:
            error_list.append(elem)
    if len(error_list) != 0:
        print(f" '{''.join(error_list)}' is not allowed")
        return -1
